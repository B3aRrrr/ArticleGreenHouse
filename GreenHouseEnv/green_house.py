...
class GreenHousePCSEEnv(PCSEEnv):
    ...
    def __init__(self,
        ...
        # general
        self.EC = self._get_default_space('EC',low_high_dict).sample()
        self.pH = self._get_default_space('pH',low_high_dict).sample()
        self.T_soil = self._get_default_space('T_soil',low_high_dict).sample()
        self.humid_soil = self._get_default_space('humid_soil',low_high_dict).sample()
        self.dV = 0
    ...
    def _apply_action(self, action):
        ...
        self.EC = EC(
                    ec_tds_0=self.EC,
                    ec_tds_solution=self.mixture['EC'],
                    V=self.V,
                    V_soil=self.R**2*self.h * np.pi,dt=self._timestep) 
        self.pH = pH(
            ph_0=self.pH,
            pH_solution=self.mixture['pH'],
            V=self.V,
            V_soil=self.R**2*self.h * np.pi,dt=self._timestep) 
        #endregion
        #region 4. Apply Temp and humid
        self.T_soil = temp_soil(
            dt=1/24*self._timestep, 
            T_soil_0=self.T_soil, 
            T_air=T_air
            )
        self.humid_soil += calculate_diff_eq(
            dt=1/24*self._timestep,
            RH=self.humid_soil,
    ...
    def _get_default_space(self,name,low_high_dict) -> gym.spaces.Space:
        return gym.spaces.Box(low_high_dict[name][0], low_high_dict[name][1], shape=(1,))
    ...
    def _get_reward(self) -> float:
        ...
        new_output['T_soil'] = self.T_soil
        new_output['humid_soil'] = self.humid_soil
        new_output['pH'] = self.pH
        new_output['EC'] = self.EC
        ...
        def calc_loc_rew(state_params,param_name,best_r):
          return np.clip(np.exp(-(state_params[param_name] - np.mean(self.optimal_space[param_name]))**2),-1,1)*best_r
        
        reward_list = np.array([calc_loc_rew(self.optimal_space,key,100) for key in check_elems])
        reward_list = np.append(
            reward_list,
            np.clip(np.exp(-(self.dV/self.V)**2), -1, 1) * 100
        )
